...
class UserModel extends AdminModel
{
    ...
    public function saveItem($params = null, $options = null)
    {
        ...
        if ($options['task'] == 'change-password-of-admin') {
            $newPass = md5($params['new_password']);
            self::where('id', $params['id'])->update(['password' => $newPass]);
        }
    }
    ...
    public function checkPass($params = null, $options = null)
    {
        $result = false;
        if ($options['task'] == 'check-pass') {
            $query = self::select('password')->where('id', $params['id'])->first();
            $oldPass = md5($params['old_password']);
